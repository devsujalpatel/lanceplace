...
const isAccountRole = (value: unknown): value is AccountRole =>
  typeof value === "string" &&
  accountRoles.some((accountRoles) => accountRoles === value);
...
export const isAuthenticated: RequestHandler = asyncHanlder(
  async (request, response, next) => {
    const token = getBearerToken(request?.headers.authorization);
    const requestRole = request.body?.role ?? request?.query.role;

    if (!token) {
      throw new ApiError(401, "Please do login to access this API!");
    }
    if (!isAccountRole(requestRole)) {
    ...
    try {
      claims = await verifyToken(token, {
        secretKey: env.clrekSecretKey,
      });
    } catch (error) {
      throw new ApiError(401, "Authentication token is invalid or expired.");
    }
    const accountAuth = await isAccountAvailable(requestRole, claims.sub);
